...
class equipo
{
	...
	public function RegEqp(equipo $data)
	{
		try
		{
			//Sentencia SQL.
	...
	public function ObtCrg()
	{
		try
		{
			...
			$stm = $this->pdo->prepare("SELECT * FROM Cargo WHERE CodCrg between 5 and 6");
	...
	public function ObtEmp()
	{
		try
		{
			$result = array();
			//Sentencia SQL para selección de datos.
			$stm = $this->pdo->prepare("EXEC ups_EmpObt");
			//Ejecución de la sentencia SQL.
			$stm->execute();
			//fetchAll — Devuelve un array que contiene todas las filas del conjunto
			//de resultados
